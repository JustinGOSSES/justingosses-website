...
at least a little bit, the experience of a field trip.
This document poses the question of whether we can create a bot that for any given point location expressed 
...
As an example, if you just ask ChatGPT to "Describe the geology of [insert a city]", the answer it gives varies in accuracy. 
For Port Clinton, Ohio, it tends to be do well, perhaps becaues the only writing that involves both "Geology" and 
"Port Clinton, Ohio" is about the geology of Port Clinton. In constrast, the same question for Houston, Texas gets 
...
By combinging deterministitic APIs, LLMs API calls with different prompts, and various validation and data cleaning 
...
There's a variety of names for this that are starting to appear. One is "augmented retrieval". Some of these approaches use a combination of 
keyword search, vector space search, and LLMs to find the most relevant information and then reorganize it into a form that is more useful 
to a user, often while pointing back to the original sources for more information and double checking. 

For our approach, we'll try to rely 
on the LLM as little as possible for actual geologic information and instead use it to help us identify what is relevant information 
and then reorganize the data & narrative information into a format and presentation style that serves our purposes best.
...
After testing this across several countries, I convince myself the answers were reliable. It was faster to do this then 
...
What now controls the productivity curve for programmatic descriptions of geology at point X are two related constraints. 
...
There's also the related but somewhat separate problem of is the deterministic data API accurate? Macrostrat is far and away the 
...
If the deterministic data sources are flawed, the LLM filtered information will be wrong as well.
...
I also felt much more productive. I am excited to see what happens next, and I'm writing this in hope that 
others might [try to build on the work a little more](https://github.com/JustinGOSSES/LAGDAL).
